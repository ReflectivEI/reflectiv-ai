/**
 * Role Play Mode Controller
 * Backend mode: "role-play"
 * Workflow: Disease + HCP → plain text HCP responses (1-3 sentences, no bullets)
 * Handles per-mode history and role-play-specific payload building
 */
...
export function createModule({ bus, store, register }){
  ...
  async function handleSend(){
    ...
    try{
      ...
      // Handle _coach data if present (plain text, no bullets)
      if(data._coach){
